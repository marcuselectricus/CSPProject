...
C NAMFIL IS THE EXTERNAL NAME OF THE BINARY EPHEMERIS FILE
      ...
      INTEGER(4)    :: nPath
C  ----------------------------------------------------------------
C  Get the JPL Ephemeris file from the environment variable
      CALL GET_ENVIRONMENT_VARIABLE('JPL_DIR',Path,nPath)
      NAMFIL = Path(1:nPath)//'\JPLEPH'
C  ----------------------------------------------------------------
C  *****************************************************************
...
C
C  -----------------------------------------------------------------
C      NRECL = 1  !INTEL Fortran on Windows
       NRECL = 4  !  GNU Fortran on Windows
...
C   FIND THE RECORD SIZE USING THE INQUIRE STATEMENT
      ...
      INQUIRE(FILE=NAMFIL,RECL=IRECSZ,IOSTAT=STATUS,ERR=1000)

C IF 'INQUIRE' DOES NOT WORK, USUALLY IRECSZ WILL BE LEFT AT 0

      IF(IRECSZ .LE. 0) THEN
        WRITE(*,*) ' INQUIRE STATEMENT DID NOT WORK '//TRIM(NAMFIL)
        WRITE(*,FMT='("  Status = ",Z8.8)')Status
        IRECSZ = 2036
      END IF

      KSIZE=IRECSZ/NRECL
C  -----------------------------------------------------------------
      WRITE(*,FMT='(" JPL_TestEph.FSIZER1 IRECSZ =", I4)') IRECSZ
      WRITE(*,FMT='(" JPL_TestEph.FSIZER1 NRECL  =", I4)') NRECL
      WRITE(*,FMT='(" JPL_TestEph.FSIZER1 KSIZE  =", I4)') KSIZE
C  -----------------------------------------------------------------

      RETURN
 1000 PRINT *,' FILE ERROR : ',NAMEFIL
      PRINT *,' IOSTAT:      ',status
      PRINT *,' Shutting Down '
      RETURN
...
C  THE SUBROUTINE SETS THE VALUES OF  NRECL, KSIZE, NRFILE, AND NAMFIL.
      ...
      LOGICAL(4)    :: Exists
C  ----------------------------------------------------------------
C  Get the JPL Ephemeris locally or from the environment variable
      INQUIRE(FILE="JPL\JPLEPH",EXIST=Exists)
      IF(Exists) THEN
        Path  = "JPL"
        nPath = LEN_TRIM(Path)
      ELSE
...
C     08-MAR-2025 | MA | SCR 150: Remove unused label raised as warning by GNU Fortran
...
C        CALL FSIZER1(NRECL,KSIZE,NRFILE,NAMFIL)  ! INTEL Fortran Windows
C        CALL FSIZER2(NRECL,KSIZE,NRFILE,NAMFIL)
         CALL FSIZER3(NRECL,KSIZE,NRFILE,NAMFIL)  !   GNU Fortran Windows
...
C
      ...
      DOUBLE PRECISION SS(3),CVAL(NMAX),ZIPS(2)
